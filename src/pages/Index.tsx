    ...
    <div className="relative overflow-hidden bg-white">
      ...
      <div className="absolute top-0 left-0 w-full h-full opacity-[0.03] pointer-events-none">
        <div className="grid grid-cols-12 gap-4 h-full">
          {[...Array(48)].map((_, i) => (
            <div key={i} className="border border-slate-900 h-24 w-full rounded-xl"></div>
          ))}
        </div>
      </div>
    </div>
...
function ProductDetailView({ product, calculatePrice, onBack, onAdd }: any) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-20">
      <button
        onClick={onBack}
        className="flex items-center gap-3 text-slate-400 hover:text-blue-600 mb-16 font-black tracking-[0.3em] text-[10px] uppercase transition-colors"
      >
        ...
        <div className="py-6">
          ...
          <div className="bg-slate-900 rounded-[3.5rem] p-14 text-white shadow-3xl">
            <div className="flex items-center justify-between mb-14">
              ...
              <div className="flex items-center gap-10 bg-white/5 p-3 rounded-[2.5rem] border border-white/10">
                <button
                  ...
                  className="p-5 hover:bg-white/10 rounded-2xl transition-all active:scale-90 text-blue-400"
                >
                  <Minus size={24} />
                </button>
            ...
            <div className="mb-14 border-t border-white/10 pt-12">
              <p className="text-slate-500 text-[10px] mb-4 font-black uppercase tracking-[0.4em]">
                Inversión Estimada Parcial
              </p>
  ...
  if (sent)
    return (
      <div className="py-60 px-4 text-center max-w-3xl mx-auto">
        ...
        <button
          onClick={onBack}
          className="bg-slate-900 text-white font-black py-7 px-16 rounded-[2.5rem] text-xl hover:bg-slate-800 transition-all shadow-2xl"
        >
    ...
    <div className="max-w-7xl mx-auto px-4 py-20">
      ...
      <div className="grid lg:grid-cols-3 gap-20">
        <div className="lg:col-span-2 space-y-12">
          {cart.length === 0 ? (
            ...
            cart.map((item: any) => (
              <div
                key={item.cartId}
                className="bg-white border border-slate-100 p-12 rounded-[4rem] flex items-center gap-12 shadow-sm hover:shadow-2xl transition-all group"
              >
                ...
                <div className="flex-1">
                  <h4 className="font-black text-slate-900 text-3xl mb-3 tracking-tighter leading-none">
                    {item.datos_generales?.nombre}
                  </h4>
                  <p className="text-[10px] text-blue-600 font-black uppercase tracking-[0.4em] mb-6">
                    {item.proveedor_nombre}
                  </p>
                  <div className="flex gap-14">
                    <div>
                      <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest mb-2 text-center">
                        Piezas
                      </p>
                ...
                <button
                  ...
                  className="text-slate-200 p-6 hover:text-red-500 hover:bg-red-50 rounded-[2rem] transition-all"
                >
                  <Trash2 size={40} />
                </button>
              </div>
        ...
        <div className="bg-slate-900 rounded-[4.5rem] p-16 text-white h-fit sticky top-32 shadow-[0_60px_100px_-20px_rgba(0,0,0,0.4)]">
          <h3 className="text-2xl font-black mb-12 border-b border-white/5 pb-10 text-blue-400 uppercase tracking-[0.4em] text-center italic leading-none">
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            <input
              ...
              required
              className="w-full bg-white/5 border-white/5 rounded-2xl p-6 font-bold text-sm focus:ring-2 focus:ring-blue-600 outline-none transition-all focus:bg-white/10"
            />
            <input
              name="e"
            ...
            <div className="pt-12 border-t border-white/5 mt-12 text-center">
              <span className="text-slate-500 font-black uppercase text-[10px] tracking-[0.4em] block mb-4">
                Inversión Estimada Total
              </span>
              <span className="text-7xl font-black text-emerald-400 tracking-tighter mb-14 block leading-none">
              ...
              >
                {sending ? (
                  ...
                  <>
                    <Building2 size={24} /> ENVIAR SOLICITUD
                  </>
                )}
              </button>
              <div className="mt-8 flex items-center justify-center gap-2 text-slate-600">
                ...
                <span className="text-[10px] font-bold uppercase tracking-widest italic">
                  Precios antes de IVA y marcaje
                </span>
              </div>
            </div>
          </form>
