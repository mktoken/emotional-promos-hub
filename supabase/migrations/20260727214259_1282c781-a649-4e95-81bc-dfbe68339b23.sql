-- ============================================================
-- FASE 1A.1 — Ajuste mínimo Motor V2 (draft, no activo)
-- Cambios:
--   1) Eliminar warning general 'pack_multiple_not_configured'.
--   2) Mantener pack_multiple = 1 como comportamiento normal.
--   3) Corregir GRANTs reales de ambas funciones V2.
--
-- Nota técnica:
--   Los múltiplos especiales para tazas de cerámica quedan
--   fuera de esta fase y NO deben inferirse mediante
--   taxonomía, categoría, subcategoría, nombre, descripción
--   ni keywords. Se implementarán cuando exista una fuente
--   confiable y explícita del múltiplo por producto.
--
-- Rollback:
--   Para revertir esta migración basta con volver a aplicar
--   la definición previa de calculate_product_price_v2 con
--   `v_pack_warning boolean := true;` y restaurar los grants
--   anteriores:
--     GRANT EXECUTE ON FUNCTION
--       public.calculate_product_price_v2(uuid, integer, uuid)
--       TO authenticated;
--     GRANT EXECUTE ON FUNCTION
--       public.admin_preview_product_price_v2(uuid, integer, uuid)
--       TO authenticated;
--   El rule set draft NO se modifica en esta migración.
-- ============================================================
...
CREATE OR REPLACE FUNCTION public.calculate_product_price_v2(
 ...
 RETURNS TABLE(
   ...
   tax_included boolean, applied_level integer,
   applied_multiplier numeric, applied_unit_cost numeric,
   adjusted_unit_cost numeric, applied_cost_factor numeric,
   applied_provider_code text, source_oferta_id uuid,
   suggested jsonb
 )
 LANGUAGE plpgsql
...
DECLARE
  ...
  -- FASE 1A.1: pack_multiple = 1 es el comportamiento normal.
  -- No se emite advertencia salvo que en el futuro exista una
  -- fuente confiable del múltiplo y se detecte un uso inválido.
...
BEGIN
  ...
  applied_unit_cost := v_scale.unit_cost; adjusted_unit_cost := v_adjusted_cost;
...
COMMENT ON FUNCTION public.calculate_product_price_v2(uuid, integer, uuid) IS
  'Motor V2 interno de precio de producto (draft). Fase 1A.1: pack_multiple=1 es normal, sin warning. Los múltiplos especiales para tazas de cerámica NO deben inferirse por taxonomía ni texto.';

-- Grants: interna solo para service_role
REVOKE ALL ON FUNCTION public.calculate_product_price_v2(uuid, integer, uuid)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.calculate_product_price_v2(uuid, integer, uuid)
  TO service_role;

-- Grants: admin preview solo para authenticated (el cuerpo exige is_staff)
REVOKE ALL ON FUNCTION public.admin_preview_product_price_v2(uuid, integer, uuid)
  FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.admin_preview_product_price_v2(uuid, integer, uuid)
  TO authenticated;
