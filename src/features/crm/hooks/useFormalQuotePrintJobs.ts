...
export function useFormalQuotePrintJobs(formalQuoteId: string | undefined) {
  ...
  const items = useQuery({
    ...
    enabled,
    queryFn: (): Promise<FormalQuotePrintJobItem[]> => listPrintJobItems(formalQuoteId!),
  });

  const components = useQuery({
    queryKey: ["formal_quote_print_job_components", formalQuoteId],
    enabled,
    queryFn: (): Promise<FormalQuotePrintJobComponent[]> => listPrintJobComponents(formalQuoteId!),
  });
  ...
  const updateJob = useMutation({
    mutationFn: ({ id, values }: { id: string; values: FormalQuotePrintJobUpdate }) => updatePrintJob(id, values),
    onSuccess: invalidateAll,
  });
  ...
  const assignItem = useMutation({
    mutationFn: (values: FormalQuotePrintJobItemInsert) => assignItemToPrintJob(values),
    onSuccess: invalidateAll,
  });
  ...
  const createComponent = useMutation({
    mutationFn: (values: FormalQuotePrintJobComponentInsert) => createPrintJobComponent(values),
    onSuccess: invalidateAll,
  });

  const updateComponent = useMutation({
    mutationFn: ({ id, values }: { id: string; values: FormalQuotePrintJobComponentUpdate }) =>
...
export type UseFormalQuotePrintJobsReturn = ReturnType<typeof useFormalQuotePrintJobs>;
