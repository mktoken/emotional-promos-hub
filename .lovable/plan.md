...
## Pre-chequeos (ya ejecutados, solo lectura) — PASS
...
## Gates obligatorios
`status=completed`; `processed_count=candidate_count`; `error_count=0`; `unresolved_count=0`; suma de estados = `candidate_count`; shadow = 0 filas; `catalog_price_cache` = 1524; `productos_publicos` y `catalog_search_products` sin cambios; 1 rule set activo; draft V2 inactivo; sin Publish.
